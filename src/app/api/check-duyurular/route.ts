...
export async function POST(request: Request) {
  try {
    ...
    const lockKey = "cron_job_lock";
    const lockTimeout = 60; // Lock timeout in seconds

    // Attempt to acquire the lock
    const lockAcquired = await redis.set(lockKey, "locked", { nx: true, ex: lockTimeout });
    if (!lockAcquired) {
      return NextResponse.json({
        success: false,
        message: "Cron job zaten çalışıyor.",
      });
    }
